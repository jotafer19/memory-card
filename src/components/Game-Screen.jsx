...
export default function GameScreen({ pokemonData }) {
  ...
  function handleShowCards() {
    ...
    if (
      !selectedCards.some((selectedCard) =>
        availableCards.some(
          (availableCard) => availableCard.id === selectedCard.id,
        ),
      )
    ) {
      console.log("hey");
  ...
  function handleGameOver(event) {
    ...
    if (clickedCards.includes(cardID)) {
      ...
      setScore((prevScore) => {
        ...
        // if (updateScore === pokemonData.length) {
        //   setIsGameOver(true);
        // }
  ...
  return (
    <div className="game screen">
      <div className="score-container">{score + " / " + pokemonData.length}</div>
